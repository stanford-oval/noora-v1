  ...
  return (
    ...
    >
      <div className="border-b-2 border-gray-400 py-4">
        ...
        {/* Focus toggle */}
        {/* {!isIOS && (
          <div className="text-gray-600 text-center m-auto mt-1 text-sm">
            <span>
              <LockOpenIcon className="h-4 w-4 -mt-0.5 inline-block" /> practice{" "}
            </span>
            <Switch
              ...
              onChange={() => {
                convoState.setValue((cs: any) => ({
                  ...cs,
                  researchMode: { focused: !cs.researchMode.focused },
                }));
              }}
              className={clsx(
                convoState.value.researchMode.focused
                  ? "bg-noora-primary"
                  : "bg-gray-200",
                "relative inline-flex flex-shrink-0 h-5 w-9 border-2 border-transparent mx-1.5 -bottom-0.5  rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-0"
              )}
            ...
            <span>
              targeted{" "}
              <LockClosedIcon className="h-4 w-4 -mt-0.5 inline-block" />
            </span>
          </div>
        )} */}
      </div>
            ...
            {({ open }) => {
              ...
              return (
                <div className="mx-auto w-full rounded-2xl bg-white py-1">
                  <Disclosure.Button
                    className={`flex w-full justify-between rounded-lg bg-gray-200 px-4 py-2 text-left text-sm font-medium text-noora-secondary hover:bg-gray-300 focus:outline-none focus-visible:ring focus-visible:ring-opacity-75 ${
