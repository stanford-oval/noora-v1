...
export function BufferScreen() {
  return (
    <div
      className="relative bg-[#ECEFF5] bg-cover bg-no-repeat bg-center overflow-hidden min-h-screen flex flex-col justify-center align-center"
      id="homeHero"
    ></div>
  );
}
...
export function LogInScreen() {
  return <Login logInInitialization={true} />;
}

export function UserCreationScreen() {
  return <Login logInInitialization={false} />;
}
...
const Login = ({ logInInitialization }) => {
  ...
  const onLogin = (e) => {
    ...
    if (logInHook) {
      ...
      if (authCode == process.env.CREATE_USER_KEY) {
        // set auth_code in environment
        createUserWithEmailAndPassword(
          auth,
          `${email}@noora.stanford.edu`,
          password
        )
          .then((userCredential) => {
            // Signed in
      ...
      <div className="relative z-10">
        <div className="pb-6 text-slate-700 flex flex-col gap-y-1 md:gap-y-2 text-center">
          <h1 className="inline bg-gradient-to-r from-noora-primary-dark via-noora-secondary-light to-noora-primary-dark1 bg-clip-text font-bold text-4xl xl:text-5xl tracking-tight">
            {logInHook ? "Log in to" : "Sign up for"}{" "}
            <span className="text-noora-primary">Noora.</span>
          </h1>
          {/* <p>
        ...
        <div className=" top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-red-200 p-6 rounded-md w-1/2">
          <form className="mx-auto text-center">
            ...
            <div>
              <button
                className="rounded-lg py-2 p-4 px-8 outline-none bg-noora-primary hover:bg-noora-primary-dark text-white mx-auto mt-4 text-lg font-medium flex justify-center"
                onClick={onLogin}
              >
